...
void NewProjectAudioProcessor::processBlock(juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
	...
	for (int i = 0; i < bufferSizeTest; ++i)
	{
		...
		for (int d = 0; d < 15; d++)
		{
			...
			int delayTimeValue = calculateAddress(rowInput, columnInput, lastSampleRate);
			int readIndex = writeArray[delayTimeValue];
			int writeIndex = writeArray[writeAddress];
			float feedbackDelayTime = writeIndex - readIndex;
			...
			/*auto feedbackReadPosition = static_cast<int>(writePositionTest - (getSampleRate() * feedbackDelayTime));
			int numSamplesToRead{ 1 };
			if (feedbackReadPosition < 0)
			{
				feedbackReadPosition += delayBufferSizeTest;
			}
			if (feedbackReadPosition + numSamplesToRead < delayBufferSizeTest)
			{
				mFeedbackBuffer.addFromWithRamp(0, i, delayBufferTest.getReadPointer(0, feedbackReadPosition), numSamplesToRead, feedbackSampleGain, feedbackSampleGain);
			}
			else
			{
				//mFeedbackBuffer.addFromWithRamp(0, i, delayBufferTest.getReadPointer(0, feedbackReadPosition), numSamplesToRead, feedbackSampleGain, feedbackSampleGain);
				auto samplesFromStart = feedbackReadPosition - delayBufferSizeTest;
				mFeedbackBuffer.addFromWithRamp(0, i, delayBufferTest.getReadPointer(0, 0), numSamplesToRead, feedbackSampleGain, feedbackSampleGain);
			}*/
	...
	for (int channel = 0; channel < totalNumOutputChannels; ++channel)
	{
		...
		if (channel == 0)
		{
			for (int d = 0; d < 3; ++d)
			{
				float outputDelayTime = ((mProgramID * outputDelayArray[d]) + outputDelayArray[d + 8] + adjustablePreDelay) * 0.001f;
				float outputDelayGain = outputGainArray[d] * outputDelayGainMult;
				readFromBuffer(delayBufferSizeTest, bufferSizeTest, channel, mOutputBuffer, delayBufferTest, outputDelayTime, outputDelayGain);
			}
		}
		//right output taps
		if (channel == 1)
